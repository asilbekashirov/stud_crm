      ...
      <ul className="flex flex-col mt-4">
        {role === "admin" ? 
          adminSidebar.map((menubar) => (
            <MenuItem key={menubar.name} {...menubar} prefix="admin" isActive={router.route === `/admin/${menubar.route}`} />
          )) :
          sidebar.map((menubar) => (
          <MenuItem key={menubar.name} {...menubar} prefix="dashboard" isActive={router.route === `/dashboard/${menubar.route}`} />
        ))}
      </ul>
    </aside>
  );
