  ...
  const registerUser = async (data: IRegisterData) => {
    ...
    if (res?.status === 201) {
      ...
      dispatch(
        successAlert({
          message:
            "Account successfully created. Open your email and activate your account to get full access.",
        })
      );
    } else {
      dispatch(
    ...
    <div className="flex flex-col min-h-screen justify-center items-center">
      <form onSubmit={handleSubmit(registerUser)}>
        ...
        <Button
          text="Submit"
          type="submit"
          wrapperClassName="mt-3 bg-primary-900 w-full"
          afterIcon="iconamoon:send-duotone"
        />
        <p className="pt-2 text-center">
          Already a user?{" "}
          <Link to="/login" className="text-secondary-800">
            Login instead
          </Link>{" "}
        </p>
