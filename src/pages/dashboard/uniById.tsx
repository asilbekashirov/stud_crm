...
const UniversityDetailPage = () => {
    ...
    const {id} = useParams()
    const {isLoading, data} = useQuery({queryKey: ["university-by-id", id], queryFn: id ? () => api.getUniversityById(id as string) : undefined})
    const universityInfo = data?.data.university || null

    return (
        <main className="">
            <div className="w-full h-96">
                <img className="w-full h-full object-cover" src={"http://localhost:5000" + universityInfo?.image as string} alt={universityInfo?.name.en} />
            </div>
